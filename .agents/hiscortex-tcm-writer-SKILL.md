...
### 0.2 กลยุทธ์การจัดเก็บแบบ Versioning / Snapshot รายการ์ด (File Snapshot Strategy)

สำหรับ **โหมด A (Local File Agent)** เพื่อไม่ให้ประวัติการเพิ่มเทสเคสของการ์ดเก่าสูญหาย และเพื่อให้ผู้ใช้สืบค้นและเปรียบเทียบย้อนหลังได้ง่าย ให้ปฏิบัติตามกลยุทธ์ต่อไปนี้:
- **ห้ามทำการแก้ไขทับไฟล์ Excel เดิมของการ์ดใบก่อนหน้าโดยตรง** หรือย้าย/เปลี่ยนชื่อไฟล์แบบเดี่ยวๆ
- **ขั้นตอนการสร้าง Snapshot สำหรับงานใหม่:**
  1. ค้นหาไฟล์ Excel ล่าสุดในระบบ (เช่น `TCM_{OldCardID}_Temp.xlsx` หรือไฟล์ template เริ่มต้น)
  2. ทำสำเนา (Copy) ไฟล์ล่าสุดนั้น แล้วเปลี่ยนชื่อไฟล์ใหม่ตามรหัสการ์ดปัจจุบัน เช่น `TCM_{CurrentCardID}_Temp.xlsx`
  3. บันทึกข้อมูลเทสเคสเฉพาะของการ์ดปัจจุบันลงในไฟล์ใหม่ที่คัดลอกมานี้
  4. วิธีนี้จะรักษาไฟล์ `TCM_{OldCardID}_Temp.xlsx` ตัวเก่าไว้ในสภาพเดิมไม่เปลี่ยนแปลง และส่งผลให้ไฟล์ใหม่เก็บประวัติสะสมทั้งหมด + งานใหม่ของการ์ดปัจจุบัน
...
10. **ใช้กลยุทธ์ Snapshot รายการ์ดเสมอ:** สำหรับโหมด A ห้ามใช้วิธีเปลี่ยนชื่อไฟล์ (mv/rename) ตรงๆ แต่ต้องทำการคัดลอก (cp/copy) ไฟล์ Excel ล่าสุดก่อน แล้วนำมาเปลี่ยนชื่อตามการ์ดเป้าหมาย เพื่อรักษาประวัติไฟล์ Excel ของการ์ดใบเก่าไว้ให้คงสภาพเดิมเสมอ
...
1. **คัดลอกไฟล์ต้นทางสร้าง Snapshot ใหม่ก่อนเสมอ:** ทำการ Copy ไฟล์ Excel ล่าสุดในระบบ (เช่น `TCM_{OldCardID}_Temp.xlsx`) ไปยังชื่อไฟล์ตามรหัสการ์ดปัจจุบัน (เช่น `TCM_{CurrentCardID}_Temp.xlsx`) ก่อนทำการแก้ไขใดๆ เพื่อรักษาประวัติการ์ดเดิม
2. โหลดไฟล์ใหม่ที่สร้างด้วย `load_workbook(path, data_only=False)` เสมอ — `data_only=False` สำคัญมาก ป้องกันสูตรเดิมถูกแทนที่ด้วยค่า hardcode ตอนเซฟ
3. หาแถวว่างที่ถูกต้อง: สแกนจากแถว 2 ของ `1. Master_TCM` ลงมา หาแถวแรกที่คอลัมน์ A (TC ID) ว่างแต่คอลัมน์ D ยังมีสูตรอยู่
4. **ต้อง unhide แถวที่เขียนด้วยเสมอ** (`ws.row_dimensions[n].hidden = False`) — แถวว่างที่เตรียมไว้ล่วงหน้าถูกซ่อนไว้ (ดูหัวข้อ 4 กฎเหล็กเรื่องแถวว่าง) ถ้าไม่ unhide ผู้ใช้จะมองไม่เห็นแถวใหม่แม้เขียนสำเร็จแล้ว
5. ถ้าต้องเพิ่มแถวใหม่เพราะไม่มีแถวว่างเหลือ ต้องขยาย range ของ Excel Table ด้วย (เช่น `ws.tables["Master_TCM"].ref = f"A1:Y{new_max_row}"`) เพื่อให้ฟอร์แมต/คุณสมบัติตารางขยายถูกต้อง
6. คัดลอกสูตรสำหรับคอลัมน์ D, P, R, T จากแถวก่อนหน้า แทนการพิมพ์ค่า literal ถ้าต้องสร้างแถวใหม่:
   - D: `="IPD - " & C{row}`
   - P: `=IF(O{row}<>"",IF(OR(P{row}="",P{row}=0),NOW(),P{row}),"")`
   - R: `=IF(Q{row}<>"",IF(OR(R{row}="",R{row}=0),NOW(),R{row}),"")`
   - T: `=IF(S{row}<>"",IF(OR(T{row}="",T{row}=0),NOW(),T{row}),"")`
7. เซฟไฟล์กลับที่ path เดิมด้วย `wb.save(path)`
...
# 1. ทำการ Copy สร้าง Snapshot ของการ์ดใบใหม่ก่อน
old_file = "./workbooks-tc/workbooks-file/TCM_NUH-1199_Temp.xlsx"
new_file = "./workbooks-tc/workbooks-file/TCM_NUH-713_Temp.xlsx"
shutil.copy(old_file, new_file)

# 2. เปิดและอัปเดตไฟล์ใหม่ที่คัดลอกมา
wb = openpyxl.load_workbook(new_file, data_only=False)
